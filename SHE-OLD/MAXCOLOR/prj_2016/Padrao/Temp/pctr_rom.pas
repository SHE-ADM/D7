...
begin
  ...
  begin
    ...
    if not Eof then
    oExceptin(Nil,'Pedido não Encontrado !');

    AIDPK := Current.Vars[0].AsString;
    ADEPK := Current.Vars[1].AsString;
    ADTPK := Current.Vars[2].AsDateTime;
  end;
  ...
  try
    try
      ...
      begin
        ...
        ibSP.ParamByName('AIDPK').Value := AIDPK;
        ibSP.ParamByName('ADEPK').Value := ADEPK;
        ibSP.ParamByName('ADTPK').Value := ADTPK;

        ibSP.ParamByName('AIDCD').Value := cadastroROM_CCLI.AsInteger;
        ibSP.ParamByName('ADECD').Value := cadastroCLI_FANT.AsString;
        ibSP.ParamByName('AIDCV').Value := cadastroROM_CVEN.AsInteger;
        ibSP.ParamByName('ADECV').Value := cadastroUSU_DUSU.AsString;

        ibSP.ParamByName('AIDCP').Value := rom_iteROM_CPRO.AsInteger;
        ibSP.ParamByName('AQTDE').Value := rom_iteROM_QTDE.AsFloat;
        ibSP.ExecProc;
    ...
    begin
      ...
      SQL.Add('WHERE  ROM_CDEP = ''' + RECParametros.ID    + '''');
      ...
      SQL.Add('WHERE  ROM_CDEP = ''' + RECParametros.ID    + '''');
      SQL.Add('AND    ROM_CCAB = ''' + CadastroID.AsString + '''');
      ExecQuery;

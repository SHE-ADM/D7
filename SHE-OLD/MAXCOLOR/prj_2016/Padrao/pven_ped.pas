...
begin
  try
    ...
    try
      ...
      begin
        CEIDCD.Tag := frmctr_ped.CadastroROM_CCLI.AsInteger;
        ...
        EDDEPK.Text := frmctr_ped.CadastroROM_DERO.AsString;
        edcdcx.Text := frmctr_ped.CadastroROM_CDCX.AsString;
        APED_DEST   := frmctr_ped.CadastroROM_STFI.AsString;
        ...
        latdsc.Caption := '%';
        CEPDSC.Value   := frmctr_ped.CadastroROM_PDSC.AsFloat;

        cbdtra.Text := frmctr_ped.CadastroROM_CTRA.AsString;
        CEVFRT.Value:= frmctr_ped.CadastroROM_VFRT.AsFloat;

        EMINFADCAD.Text := frmctr_ped.CadastroROM_OBSE.AsString;
        ...
        try
          ...
          begin
            ...
            EdicaoITEM.Value := frmctr_ped.rom_iteROM_ITEM.AsInteger;
            EdicaoIDCP.Value := frmctr_ped.rom_iteROM_CPRO.AsInteger;
            EdicaoDGCP.Value := frmctr_ped.rom_iteROM_DCOR.AsString;
            EdicaoDECP.Value := frmctr_ped.rom_iteROM_DPRO.AsString;
            EdicaoUCOM.Value := frmctr_ped.rom_iteROM_DUNI.AsString;

            EdicaoQTDE.Value := frmctr_ped.rom_iteROM_QTDE.AsFloat;
            EdicaoQTRL.Value := frmctr_ped.rom_iteROM_QTRL.AsInteger;

            EdicaoVPRC_PAD_INI.AsFloat := frmctr_ped.rom_iteROM_UNIT.AsFloat;
            EdicaoVPRC_PAD_FIM.AsFloat := frmctr_ped.rom_iteROM_UNIT.AsFloat;

            EdicaoVPRC_PAD.AsFloat := frmctr_ped.rom_iteROM_UNIT.AsFloat;
            EdicaoVPRC_COM.AsFloat := frmctr_ped.rom_iteROM_PREC.AsFloat;

            EdicaoPACR.Value := IFThen((CBSTCO.Text = '30/30') and (CBSTPD.Text = 'FATURADO'),5,0); { Acréscimo de 5% }

  //          EdicaoPDSC.Value := frmctr_ped.rom_iteROM_PDSC.AsFloat;
    //        EdicaoVDSC.Value := frmctr_ped.rom_iteROM_VDSC.AsFloat;

            EdicaoTSDE.Value := frmctr_ped.rom_iteROM_TOTA.AsFloat;
            EdicaoTCDE.Value := frmctr_ped.rom_iteROM_TOTA.AsFloat;
   ...
   begin
     ...
     try
       ...
       if frmcad_pro_con.Editado then
          PESQUISA_PRODUTO('PK.ID',IntToStr(frmcad_pro_con.CadastroID.AsInteger),0);
     finally
...
//        oException(Nil,'Produto ' + FrmPrincipal.PSQ_PRODEST.AsString);
     ...
     EdicaoIDCP.Value     := frmprincipal.psq_proID.AsInteger;
//     EdicaoIDSK.Value     := frmprincipal.psq_proIDSK.AsInteger;
                       
     EdicaoARTIGO.Value   := frmprincipal.psq_proPRO_CART.AsString;
     EdicaoSKU.Value      := frmprincipal.psq_proPRO_CPRO.AsString;

     EdicaoDGCP.Value     := frmprincipal.psq_proPRO_DCOR.AsString;
     EdicaoDECP.Value     := frmprincipal.psq_proPRO_DPRO.AsString;
     EdicaoCMP_PAD.Value  := FrmPrincipal.psq_proPRO_COMP.AsString;

     EdicaoUCOM.Value     := frmprincipal.psq_proPRO_DUNI.AsString;
//     EdicaoEPE_QTDE.Value := frmprincipal.psq_proEPE_QTDE.AsFloat;
  //   EdicaoEPE_QTRL.Value := frmprincipal.psq_proEPE_QTRL.AsInteger;

     EdicaoVPRC_PAD_INI.Value := FrmPrincipal.psq_pro.FieldByName('UNIT').AsFloat;
     EdicaoVPRC_PAD_FIM.Value := FrmPrincipal.psq_pro.FieldByName('UNIT').AsFloat;

     EdicaoVPRC_PAD.Value := FrmPrincipal.psq_pro.FieldByName('UNIT').AsFloat;
     EdicaoVPRC_COM.Value := FrmPrincipal.psq_pro.FieldByName('UNIT').AsFloat;

     EdicaoPACR.Value     := IFThen((CBSTCO.Text = '30/30') and (CBSTPD.Text = 'FATURADO'),5,0); { Acréscimo de 5% }
//     EdicaoPIPI.Value     := frmprincipal.psq_proPIPI.AsFloat;

     EdicaoGRP_NO.Value   := FrmPrincipal.psq_proPRO_DCOR.AsString;

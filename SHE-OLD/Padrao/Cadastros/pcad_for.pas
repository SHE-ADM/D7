...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT CF.ID "Id",FOR_FANT "Fantasia",FOR_RAZA "Razao",');
    ...
    SQL.Add('FROM   CAD_FOR AS CF');
    SQL.Add('LEFT   OUTER JOIN CAD_USU     AS Vendedores     ON (FOR_CVEN        = Vendedores.USU_CUSU)');
    SQL.Add('LEFT   OUTER JOIN CAD_REP     AS Representantes ON (FOR_CREP        = Representantes.ID)');

    SQL.Add('WHERE (CAST(CF.DTCA AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))) ');
    SQL.Add('OR     CAST(CF.DTPC AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))) ');
    SQL.Add('OR     CAST(CF.DTNF AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))))');

    SQL.Add('ORDER BY CF.ID');
    Open;
  end;
end;
...
begin
  ...
  begin
    ...
    RECPrincipal.PSQTexto      := bPesquisaNotasFiscais(RECPrincipal.PSQTexto,1);
  end;
  ...
  begin
    ...
    SQL.Add('LEFT   OUTER JOIN CAD_USU     AS Vendedores     ON (FOR_CVEN        = Vendedores.USU_CUSU)');
    SQL.Add('LEFT   OUTER JOIN CAD_REP     AS Representantes ON (FOR_CREP        = Representantes.ID)');
    SQL.Add('WHERE    CF.ID > 0');
    
    if Length(RECPrincipal.PSQTexto) >= 1 then
    SQL.Add('AND CF'+IFThen(RECPrincipal.PSQTextoField = 'ID','.ID','.FOR'+RECPrincipal.PSQTextoField)+' '+RECPrincipal.PSQWhere+' '''+Trim(RECPrincipal.PSQLike1+RECPrincipal.PSQTexto+RECPrincipal.PSQLike2)+'''');

    if not oEmpty(RECPrincipal.PSQData) then
    SQL.Add('AND      CF.FOR' + RECPrincipal.PSQData+' BETWEEN '''+formatDateTime('mm/dd/yy',RECPrincipal.PSQDT1)+''' AND '''+formatDateTime('mm/dd/yy',RECPrincipal.PSQDT2)+'''');
    SQL.Add('ORDER BY CF'     + IFThen(RECPrincipal.PSQTextoField = 'ID','.ID','.FOR'+RECPrincipal.PSQTextoField));
    Prepare;
    Open;

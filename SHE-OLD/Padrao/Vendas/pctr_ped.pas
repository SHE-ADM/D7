...
type
  Tfrmctr_ped = class(TFrmPadr2)
    ...
    procedure ACTEdicaoDevolucaoExecute(Sender: TObject);
    procedure ACTEdicaoAbatimentoExecute(Sender: TObject);
    procedure ACTEdicaoPedidoExecute(Sender: TObject);
...
procedure Tfrmctr_ped.ACTEdicaoPedidoExecute(Sender: TObject);
begin
...
begin
  ACTEdicaoPedido.HelpContext := 0; { Novo }
  ACTEdicaoPedido.HelpKeyword := EmptyStr; { Pedido }
  ACTEdicaoPedido.Tag := 0; { Cadastro }
  ACTEdicaoPedido.Execute;
end;

procedure Tfrmctr_ped.ACTMPEditExecute(Sender: TObject);
begin
  ACTEdicaoPedido.HelpContext := 1; { Edição }
  ACTEdicaoPedido.HelpKeyword := CadastroDEPK.AsString; { Pedido }
  ACTEdicaoPedido.Tag := CadastroIDPK.AsLargeInt; { Cadastro }
  ACTEdicaoPedido.Execute;
end;

procedure Tfrmctr_ped.ACTEdicaoDevolucaoExecute(Sender: TObject);
begin
  if (Cadastro.State <> dsBrowse) or (Cadastro.RecNo = 0) then
  Abort;

  ACTEdicaoPedido.HelpContext := 2; { Devolução }
  ACTEdicaoPedido.HelpKeyword := CadastroDEPK.AsString; { Pedido }
  ACTEdicaoPedido.Tag := CadastroIDPK.AsLargeInt; { Cadastro }
  ACTEdicaoPedido.Execute;
end;

procedure Tfrmctr_ped.ACTEdicaoAbatimentoExecute(Sender: TObject);
begin
  if (Cadastro.State <> dsBrowse) or (Cadastro.RecNo = 0) then
  Abort;

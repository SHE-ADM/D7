...
Type
  TFrmRelatorios = class(TForm)
    ...
    procedure ACTREL_GER_EST_UCOM_UPESOExecute(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure ACTREL_GER_EST_CRDExecute(Sender: TObject);
...
//  {$IF DEFINED(DEF_ADM)}
     ...
     qGER_EST_UCOM_UPESO, QGER_VEN_FAT_CRD;



//  {$ELSEIF DEFINED(DEF_ADM)}
//     ,qGER_EST_UCOM_UPESO, QGER_VEN_FAT_CRD;
...
begin
  ...
  REC_SHE_REL.IDPK := IntToStr(ACTRelatorio.HelpContext);
  REC_SHE_REL.IDFK := IntToStr(ACTRelatorio.HelpContext);
  ...
  REC_SHE_REL.IDPK := IFThen(((not oEmpty(REC_SHE_REL.IDPK)) and (not oEmpty(REC_SHE_REL.IDFK))),REC_SHE_REL.IDPK,'TODOS');
  REC_SHE_REL.IDFK := IFThen(((not oEmpty(REC_SHE_REL.IDPK)) and (not oEmpty(REC_SHE_REL.IDFK))),REC_SHE_REL.IDFK,'TODOS');
...
begin
  ...
  MCData1.Date := REC_SHE_REL.DTPK;
  MCData2.Date := REC_SHE_REL.DTFK;
...
begin
  REC_SHE_REL.PSQ_TPO_FC := IETipo.Descriptions[IETipo.Values.IndexOf(IETipo.Text)];
  REC_SHE_REL.PSQ_TPO_FV := IETipo.Text;
...
begin
  REC_SHE_REL.PSQ_STA_FC := IEStatus.Descriptions[IEStatus.Values.IndexOf(IEStatus.Text)];
  REC_SHE_REL.PSQ_STA_FV := IEStatus.Text;
...
begin
  if REC_SHE_REL.EP_NO <> IEEmpresa.Text then
     ...
     begin
       ...
       REC_SHE_REL.EP_RZ   := Current.Vars[2].AsString;
...
begin
  ...
  begin
    ...
    REC_SHE_REL.PSQ_DT_FC_PK := IEData.Descriptions[IEData.Values.IndexOf(IEData.Text)];
    REC_SHE_REL.PSQ_DT_FD_PK := IEData.Values[IEData.Values.IndexOf(IEData.Text)];

    if  REC_SHE_REL.Titulo = 'Listagem de Clientes' then
        REC_SHE_REL.PSQ_DT_FD_PK := 'CLI_'+REC_SHE_REL.PSQ_DT_FD_PK else
    if  REC_SHE_REL.Titulo = 'Listagem de Representantes' then
        REC_SHE_REL.PSQ_DT_FD_PK := 'REP_'+REC_SHE_REL.PSQ_DT_FD_PK;

    REC_SHE_REL.PSQ_DT_FV_PK := MCData1.Date;
    REC_SHE_REL.PSQ_DT_FV_FK := MCData2.Date;
  ...
  begin
    REC_SHE_REL.PSQ_DT_FC_PK := '';
    REC_SHE_REL.PSQ_DT_FD_PK := '';
    REC_SHE_REL.PSQ_DT_FV_PK := 0;
    REC_SHE_REL.PSQ_DT_FV_PK := 0;
...
begin
  REC_SHE_REL.PSQ_CD_FV_PK := PEC1Codigo.Text;
...
begin
  REC_SHE_REL.PSQ_CD_FV_FK := PEC2Codigo.Text;
end;
...
begin
  ...
  case TdxImageEdit(Sender).Tag of
    1: begin
         REC_SHE_REL.PSQ_PE1_TB := Trim(LeftStr(IEConsultaField,Pos('.',IEConsultaField) - 1));
         REC_SHE_REL.PSQ_PE1_FD := Trim(IEConsultaAlias + Trim(Copy(IEConsultaField,Pos('.',IEConsultaField) + 1,Length(IEConsultaField))));
         REC_SHE_REL.PSQ_PE1_FC := IEConsultaCaption;
       end;
    2: begin
         REC_SHE_REL.PSQ_PE2_TB := IEConsultaCaption;
         REC_SHE_REL.PSQ_PE2_FD := Trim(LeftStr(IEConsultaField,Pos('.',IEConsultaField) - 1));
         REC_SHE_REL.PSQ_PE2_FC := Trim(IEConsultaAlias + Trim(Copy(IEConsultaField   ,Pos('.',IEConsultaField) + 1,Length(IEConsultaField))));
       end;
    3: begin
         REC_SHE_REL.PSQ_PE3_TB := IEConsultaCaption;
         REC_SHE_REL.PSQ_PE3_FD := Trim(LeftStr(IEConsultaField,Pos('.',IEConsultaField) - 1));
         REC_SHE_REL.PSQ_PE3_FC := Trim(IEConsultaAlias + Trim(Copy(IEConsultaField   ,Pos('.',IEConsultaField) + 1,Length(IEConsultaField))));
       end;
    4: begin
         REC_SHE_REL.PSQ_PE4_TB := IEConsultaCaption;
         REC_SHE_REL.PSQ_PE4_FD := Trim(LeftStr(IEConsultaField,Pos('.',IEConsultaField) - 1));
         REC_SHE_REL.PSQ_PE4_FC := Trim(IEConsultaAlias + Trim(Copy(IEConsultaField   ,Pos('.',IEConsultaField) + 1,Length(IEConsultaField))));
       end;
  end;
...
begin
  ...
  case TdxImageEdit(Sender).Tag of
    1: begin
         REC_SHE_REL.PSQ_IE1_WHERE := IEConsultaWhere;
         REC_SHE_REL.PSQ_IE1_LKPK  := IEConsultaLike1;
         REC_SHE_REL.PSQ_IE1_LKFK  := IEConsultaLike2;
       end;
    2: begin
         REC_SHE_REL.PSQ_IE2_WHERE := IEConsultaWhere;
         REC_SHE_REL.PSQ_IE2_LKPK  := IEConsultaLike1;
         REC_SHE_REL.PSQ_IE2_LKFK  := IEConsultaLike2;
       end;
    3: begin
         REC_SHE_REL.PSQ_IE3_WHERE := IEConsultaWhere;
         REC_SHE_REL.PSQ_IE3_LKPK  := IEConsultaLike1;
         REC_SHE_REL.PSQ_IE3_LKFK  := IEConsultaLike2;
       end;
    4: begin
         REC_SHE_REL.PSQ_IE4_WHERE := IEConsultaWhere;
         REC_SHE_REL.PSQ_IE4_LKPK  := IEConsultaLike1;
         REC_SHE_REL.PSQ_IE4_LKFK  := IEConsultaLike2;
       end;
  end;
...
begin
  case TdxImageEdit(Sender).Tag of
    1: REC_SHE_REL.PSQ_PE1_FV := TdxImageEdit(Sender).Text;
    2: REC_SHE_REL.PSQ_PE2_FV := TdxImageEdit(Sender).Text;
    3: REC_SHE_REL.PSQ_PE3_FV := TdxImageEdit(Sender).Text;
    4: REC_SHE_REL.PSQ_PE4_FV := TdxImageEdit(Sender).Text;
  end;
end;
...
procedure TFrmRelatorios.ACTREL_GER_EST_UCOM_UPESOExecute(Sender: TObject);
begin
  if Assigned(QRPGER_EST_UCOM_UPESO) then QRPGER_EST_UCOM_UPESO.BringToFront else
  begin
    QRPGER_EST_UCOM_UPESO := TQRPGER_EST_UCOM_UPESO.Create(Self,REC_SHE_REL);
    QRPGER_EST_UCOM_UPESO.WinControlFormCreate(QRPGER_EST_UCOM_UPESO);
  end;
end;

procedure TFrmRelatorios.ACTREL_GER_EST_CRDExecute(Sender: TObject);
begin
  if Assigned(QRPGER_VEN_FAT_CRD) then QRPGER_VEN_FAT_CRD.BringToFront else
  begin
    QRPGER_VEN_FAT_CRD := TQRPGER_VEN_FAT_CRD.Create(Self,REC_SHE_REL);
    QRPGER_VEN_FAT_CRD.WinControlFormCreate(QRPGER_VEN_FAT_CRD);
  end;
end;
...
begin
  ...
  { Diversos }
  REC_SHE_REL.IDPK    := PEC1Codigo.Text; { Primary Key }
  REC_SHE_REL.IDFK    := PEC2Codigo.Text; { Foreign Key }
  REC_SHE_REL.QRodape := EmptyStr;    { Linha de Rodapé }
...
begin
  ...
  if ((REC_SHE_REL.Nome = 'Ranking de Produtos')             or (REC_SHE_REL.Nome = 'Vendas de Produtos') or (REC_SHE_REL.Nome = 'Vendas Mensais de Produtos') or
      (REC_SHE_REL.Nome = 'Vendas de Produtos por Clientes') or (REC_SHE_REL.Nome = 'Vendas de Produtos por Grupos de Clientes')) then
  begin
  ...
  if ((REC_SHE_REL.Nome = 'Vendas Mensais de Produtos') or (REC_SHE_REL.Nome = 'Vendas por UF') or (REC_SHE_REL.Nome = 'Vendas por Região Nacional')) then
  begin
    IEModelo.Descriptions.Clear;
    IEModelo.Descriptions.Add('Produto_X_Valor');
  ...
  if (REC_SHE_REL.Nome = 'Performance de Vendas') or (REC_SHE_REL.Nome = 'Performance de Preços') or (REC_SHE_REL.Nome = 'Tabela de Preços') then
  begin
  ...
  case REC_SHE_REL.TAGPerfil of
    0: begin
         if REC_SHE_REL.TAGNome = 1 then { Tipos de Créditos }
         begin
           IEEmpresa.Enabled := (Pos(RECUsuarios.Grupo,'DEVDIR') > 0);
           if not IEEmpresa.Enabled then
           IEEmpresa.Text := REC_SHE_REL.EP_NO;

           IENome.Values.Add('ACTREL_GER_VEN_FAT_CRD');
           IENome.Text := 'ACTREL_GER_VEN_FAT_CRD';
         ...
         begin
           ...
           IENome.Values.Add('ACTREL_GER_EST_UCOM_UPESO');
           IENome.Text := 'ACTREL_GER_EST_UCOM_UPESO';
  ...
  begin
    PNLCodigos.Tag := 1;
    REC_SHE_REL.PSQ_CD_FC_PK := 'Quantidade de Itens';
    REC_SHE_REL.PSQ_CD_FD_PK := 'First';

    REC_SHE_REL.PSQ_CD_FC_PK := 'Relevância';
    REC_SHE_REL.PSQ_CD_FD_PK := 'OrderBy';
  ...
  { Códigos }
  PNLCodigos.Visible := (not oEmpty(REC_SHE_REL.PSQ_CD_FV_PK));
  if PNLCodigos.Visible then
  begin
    GB1Codigo.Caption := '  ' + REC_SHE_REL.PSQ_CD_FV_PK + '  ';
    GB2Codigo.Caption := '  ' + REC_SHE_REL.PSQ_CD_FV_FK + '  ';
  end;
end;

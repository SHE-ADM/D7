...
type
  TQRPGER_VEN_FAT_CRD = class(TQuickRep)
...
var
  QRPGER_VEN_FAT_CRD: TQRPGER_VEN_FAT_CRD;

implementation

{$R *.DFM}

Constructor TQRPGER_VEN_FAT_CRD.Create(AOwner: TComponent; const AREC_SHE_REL: TREC_SHE_REL);
begin
...
procedure TQRPGER_VEN_FAT_CRD.WinControlFormCreate(Sender: TObject);
          procedure _Select;
          begin
            REC_SHE_REL.PSQ_EP_ID_PK := INTTOSTR(IFThen(REC_SHE_REL.EP_ID > 0,REC_SHE_REL.EP_ID,1));
            REC_SHE_REL.PSQ_EP_ID_FK := INTTOSTR(IFThen(REC_SHE_REL.EP_ID > 0,REC_SHE_REL.EP_ID,9));

            { Filtros Datas }
            if REC_SHE_REL.PSQ_DT_FC_PK <> EmptyStr then
            begin
              REC_SHE_REL.RodapeFiltro := 'Período: ' + FormatDateTime('dd/mm/yy',REC_SHE_REL.PSQ_DT_FV_PK) + ' até ' + FormatDateTime('dd/mm/yy',REC_SHE_REL.PSQ_DT_FV_FK);
            end;
            ...
            begin
              ...
              SQL.Add('''' + REC_SHE_REL.PSQ_EP_ID_PK + ''',');
              SQL.Add('''' + REC_SHE_REL.PSQ_EP_ID_FK + ''',');

              SQL.Add('''' + FormatDateTime('mm/dd/yy',REC_SHE_REL.PSQ_DT_FV_PK) + ''',');
              SQL.Add('''' + FormatDateTime('mm/dd/yy',REC_SHE_REL.PSQ_DT_FV_FK) + ''')');
              SQL.Add(' AS PK');
          ...
          begin
            { Titulo / Sub Título }
            QRLSubTitulo.Enabled := (REC_SHE_REL.Titulo <> REC_SHE_REL.SubTitulo) and (REC_SHE_REL.SubTitulo <> EmptyStr);
...
begin
  with QRPGER_VEN_FAT_CRD do
       try
         ...
         WinControlFormDestroy(QRPGER_VEN_FAT_CRD);
       end;
end;

procedure TQRPGER_VEN_FAT_CRD.WinControlFormDestroy(Sender: TObject);
begin
  try
    ...
    FreeAndNil(QRPGER_VEN_FAT_CRD);
  end;
end;

procedure TQRPGER_VEN_FAT_CRD.QRBDETALHESBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  { Devoluções }
...
procedure TQRPGER_VEN_FAT_CRD.QRBTOTALBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQRPGER_VEN_FAT_CRD.QRGGRUPOBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin

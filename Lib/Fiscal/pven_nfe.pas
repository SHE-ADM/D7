...
type
  TFrmVEN_NFE = class(TForm)
    ...
    DBGFIS_NFE_DUPNFE_DVENC: TdxDBGridDateColumn;
    DBGFIS_NFE_DUPNFE_VDUP: TdxDBGridCurrencyColumn;
    DBGFIS_NFE_DUPNFE_VDESC: TdxDBGridCurrencyColumn;
    DBGFIS_NFE_DUPNFE_VORIG: TdxDBGridCurrencyColumn;
...
uses uPrincipal, bPrincipal, pemail, pNFeConsultaSefaz, pimporta_geral;
...
begin
  try { Pesquisa }
    ...
    ACTPSQ_TAB_CFOP.HelpKeyWord := SQLPKConsulta.Current.ByName('CFOP').AsString; { Value }
    ...
    try { Sincronismo }
      ...
      try { Edição }
        ...
        begin
          if REC_SHE_DEF.FInitialize then
          begin
            PNLSyncEvent.Caption := 'Aguarde, carregando registro item nº' + SQLPKConsulta.Current.ByName('ITEM').AsString       + ' - ' + SQLPKConsulta.Current.ByName('SKU').AsString     +
          ...
          EdicaoCP_CMP_NO.Value    := SQLPKConsulta.Current.ByName('CMP_NO').AsString;
...
//    begin
//      Edicao.Append;
//      EdicaoNFE_ITEMPED.Value := SQLPKConsulta.Current.ByName('NFE_ITEMPED').AsInteger;
//      EdicaoNFE_CFOP.Value    := IFThen(REC_SHE_DEF.IDEV = 0,SQLPKConsulta.Current.ByName('NFE_CFOP').AsString,
...
//      nfe_001.Post;
//      EdicaoNFE_CPROD.Value  := SQLPKConsulta.Current.ByName('NFE_CPROD').AsString;
//      EdicaoNFE_CEAN.Value   := SQLPKConsulta.Current.ByName('NFE_CEAN' ).AsString;
//      EdicaoNFE_XPROD.Value  := SQLPKConsulta.Current.ByName('NFE_XPROD').AsString;
//      EdicaoNFE_UCOM.Value   := SQLPKConsulta.Current.ByName('NFE_UCOM' ).AsString;
//
//      EdicaoNFE_NCM.Value    := SQLPKConsulta.Current.ByName('NFE_NCM'   ).AsString;
//      EdicaoNFE_PIPI.Value   := SQLPKConsulta.Current.ByName('NFE_PIPI'  ).AsFloat;
//      EdicaoNFE_CEST.Value   := SQLPKConsulta.Current.ByName('NFE_CEST'  ).AsString;
//      EdicaoNFE_EXTIPI.Value := SQLPKConsulta.Current.ByName('NFE_EXTIPI').AsString;
//
//      EdicaoNFE_ORIG.Value   := SQLPKConsulta.Current.ByName('NFE_ORIG').AsString;
//
//      EdicaoNFE_QCOM.AsFloat := SQLPKConsulta.Current.ByName('NFE_QCOM').AsFloat;
//      EdicaoNFE_RCOM.Value   := SQLPKConsulta.Current.ByName('NFE_RCOM').AsInteger;
//
//      EdicaoNFE_PREC.Value   := SQLPKConsulta.Current.ByName('NFE_VUNCOM').AsFloat;
//      EdicaoNFE_VUNCOM.Value := SQLPKConsulta.Current.ByName('NFE_VUNCOM').AsFloat;
//
//      EdicaoNFE_NFCI.Value   := SQLPKConsulta.Current.ByName('NFE_NFCI').AsString;
//
//      EdicaoNFE_PSCN.Value   := SQLPKConsulta.Current.ByName('PSCN').AsFloat;
//      EdicaoNFE_PSBR.Value   := SQLPKConsulta.Current.ByName('PSBR').AsFloat;
//      EdicaoNFE_PSLQ.Value   := SQLPKConsulta.Current.ByName('PSLQ').AsFloat;
//      Edicao.Post;
//
//      SQLPKConsulta.Next;
//    end;
...
begin
  ...
  try
    ...
    try
      ...
      EdicaoCP_CMP_NO.Value  := Current.ByName('CMP_NO').AsString;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        SPEdicao.ParamByName('DCOR').Value           := EmptyStr;//RICARDOEdicaoCP_GRD_NO.AsString;
  ...
  try
    ...
    //RICARDOACTCAD_PRO_EST_LAN.Execute;
  ...
  try
    ...
    if CECDRO.Value > 0 then
       ...
       begin
         ...
         if (fields[0].AsString = 'BANCÁRIO') and (FIS_NFE_DUP.Fields[0].IsNull) then
...
begin
  ...
  try { Cálculo }
    ...
    try { Sincronismo }
      ...
      if REC_SHE_DEF.FInitialize then
      begin
        PNLSyncEvent.Caption := 'Aguarde, verificando a carga tributária ...';
      ...
      try { Impostos }
        ...
        begin
          if REC_SHE_DEF.FInitialize then
          begin
            PNLSyncEvent.Caption := 'Aguarde, calculando a carga tributária ...';
            Application.ProcessMessages;
          end;
...
begin
  ...
  if REC_SHE_DEF.FInitialize then
  begin
    PNLSyncEvent.Caption := 'Aguarde, verificando descontos ...';
    PNLSyncEvent.Refresh;
    SleepEx(500,False);
  end;
  ...
  begin
    PNLSyncEvent.Caption := 'Desconto calculado com sucesso !';
    PNLSyncEvent.Refresh;
    SleepEx(500,False);
  end;
end;
...
begin
  ...
  if REC_SHE_DEF.FInitialize then
  begin
    PNLSyncEvent.Caption := 'Aguarde, verificando frete ...';
    PNLSyncEvent.Refresh;
    SleepEx(500,False);
  end;
  ...
  begin
    PNLSyncEvent.Caption := 'Frete calculado com sucesso !';
    PNLSyncEvent.Refresh;
    SleepEx(500,False);
  end;  
end;
...
begin
  ...
  if REC_SHE_DEF.FInitialize then
  begin
    PNLSyncEvent.Caption := 'Aguarde, verificando seguro ...';
    PNLSyncEvent.Refresh;
    SleepEx(500,False);
  end;
  ...
  begin
    PNLSyncEvent.Caption := 'Seguro calculado com sucesso !';
    PNLSyncEvent.Refresh;
    SleepEx(500,False);
  end;  
end;
...
begin
  ...
  begin
    if REC_SHE_DEF.FInitialize then
    begin
      PNLSyncEvent.Caption := 'Aguarde, alterando a finalidade de emissão da nota fiscal para CONSUMO INTERNO ...';
      PNLSyncEvent.Refresh;
      SleepEx(500,False);
    end;
    ...
    if REC_SHE_DEF.FInitialize then
    begin
      PNLSyncEvent.Caption := 'Nota Fiscal alterada para consumidor final !';
      PNLSyncEvent.Refresh;
      SleepEx(500,False);
    end;
  end;
...
begin
  ...
  if Pos(IECFOP.Text,'59246924') > 0 then
  begin
    EDINFADNF.Lines.Clear;
...
begin
  ...
  try { Duplicatas }
    ...
    try { Sincronismo }
      if REC_SHE_DEF.FInitialize then
      begin
        PCEdicao.ActivePage := TSDuplicata; { Page Control }
      ...
      try { Edição }
        ...
        begin
          if REC_SHE_DEF.FInitialize then
          begin
            PNLSyncEvent.Caption := 'Gerando Duplicata Nº ' + CECDNF.Text + IFThen(TAB_PAGPAG_PARC.AsInteger > 1,'-' + aDUP[i],EmptyStr) + ' ...';
            Application.ProcessMessages;
            SleepEx(200,False);
          end;
...
begin
  ...
  try
    ...
    begin
      ...
      if Eof then
      begin
        MessageBox(Application.Handle,PChar(IFThen(LAIDCD.Tag = 0,'Cliente','Fornecedor') + ' não Encontrado !'),'Importação de XML',MB_ICONWARNING+MB_OK);
        Abort;
        //CADASTRA_FAVORECIDO(ANodeSec,ANodeEnd);
      end else
      begin
        { DESTINATÁRIO }
    ...
    repeat
      ...
      repeat
        ...
        begin
          ...
          EdicaoNFE_CPROD.Value     := Trim(ANodeTmp.ChildNodes['cProd'].Text);
          ...
          EdicaoNFE_VUNPAD.AsFloat  := oTextToValor(Trim(ANodeTmp.ChildNodes['vUnCom'].Text));
          EdicaoNFE_VUNCOM.AsFloat  := oTextToValor(Trim(ANodeTmp.ChildNodes['vUnCom'].Text));
          EdicaoNFE_VUNTRIB.AsFloat := oTextToValor(Trim(ANodeTmp.ChildNodes['vUnTrib'].Text));
    ...
    repeat
      ...
      begin
        CEPSLQ.Text := oStrTran(Trim(ANodeTmp.ChildNodes['pesoB'].Text),'.',',');
        CEPSBR.Text := oStrTran(Trim(ANodeTmp.ChildNodes['pesoL'].Text),'.',',');
      end;

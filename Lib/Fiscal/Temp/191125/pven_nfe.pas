...
type
  Tfrmven_nfe = class(TForm)
    ...
    ACTPSQLote: TAction;
    ACTPSQNotaFiscal: TAction;
    ACTPSQRomaneio: TAction;
    ...
    ACTXMLSefaz: TAction;
    ACTXMLLote: TAction;
    ACTCAD_PAD_EDT: TAction;
    ACTCAD_PRO_EST_LAN: TAction;
    ACTNFeDescontos: TAction;
    ACTNFeUPD: TAction;
    ACTNFeDanfe: TAction;
    ACTNFePost: TAction;
    ...
    procedure PECFOPChange(Sender: TObject);
    ...
    procedure ACTPSQNotaFiscalExecute(Sender: TObject);
    procedure ACTPSQRomaneioExecute(Sender: TObject);
    ...
    procedure ACTXMLLoteExecute(Sender: TObject);
    procedure ACTCAD_PAD_EDTExecute(Sender: TObject);
    procedure ACTCAD_PRO_EST_LANExecute(Sender: TObject);
    procedure ACTNFeUPDExecute(Sender: TObject);
    ...
    procedure ACTPSQLoteExecute(Sender: TObject);
    ...
    procedure ACTCheckNFeSeqExecute(Sender: TObject);
    procedure ACTXMLSefazExecute(Sender: TObject);
    ...
    procedure EDDECDChange(Sender: TObject);
    procedure PECFOP_NOChange(Sender: TObject);
    ...
    procedure ACTNFePrintExecute(Sender: TObject);
    procedure EEventAdminEventAlert(Sender: TObject; EventName: String;
      EventCount: Integer; var CancelAlerts: Boolean);
    procedure NFE_001BeforeEdit(DataSet: TDataSet);
    procedure NFE_001EditError(DataSet: TDataSet; E: EDatabaseError;
      var Action: TDataAction);
    procedure NFE_001DeleteError(DataSet: TDataSet; E: EDatabaseError;
      var Action: TDataAction);
    procedure NFE_001PostError(DataSet: TDataSet; E: EDatabaseError;
      var Action: TDataAction);
    procedure NFE_001UpdateError(DataSet: TDataSet; E: EDatabaseError;
      UpdateKind: TUpdateKind; var UpdateAction: TIBUpdateAction);
    procedure NFE_001CalcFields(DataSet: TDataSet);
...
uses uPrincipal, bPrincipal,
  pemail, pctr_rom, pctr_nfe, pimporta_geral, pNFeConsultaSefaz;
...
begin
  ...
  ACTNFeUPD.HelpKeyword := EmptyStr;
  ACTNFeUPD.Execute
end;
...
begin
  ...
  { VER REGISTROS }
  ACTPesquisas.Execute; { Pesquisa Principal }
end;
...
begin
  ...
  { ACCESS MANAGER }
  REC_SHE_DEF.FB_Event := 'FIS_NFE_EDT'; { Eventos }
...
begin
  ...
  if REC_SHE_DEF.FB_Event <> EmptyStr then
     try
       ...
       begin
         REC_SHE_DEF.FB_EVE_EDT := REC_SHE_DEF.FB_Event + '-' + oStrZero(RECParametros.EP_ID,3) + '-' + oStrZero(ACTEveRegister.Tag,3);
...
procedure Tfrmven_nfe.EEventAdminEventAlert(Sender: TObject;
...
procedure Tfrmven_nfe.NFE_001BeforeEdit(DataSet: TDataSet);
begin
  NFE_001Id.Tag := NFE_001Id.AsInteger;
end;
...
begin
  ...
  begin
    ...
    NFE_001NFE_ITEMPED.Value := Current.Vars[0].AsInteger + 1;
  end;

  NFE_001NFE_CDNF.Value := Trunc(CECDNF.Value);
  NFE_001NFE_CFOP.Value := LeftStr(PECFOP.Text,4);
...
begin
  ...
  begin
    ...
    { Peso Líquido}
    if  LeftStr(NFE_001NFE_UCOM.AsString,1) = 'K' then NFE_001NFE_PSLQ.Value := NFE_001NFE_QCOM.AsCurrency else
    if (LeftStr(NFE_001NFE_UCOM.AsString,1) = 'M') and (NFE_001NFE_REND.AsFloat > 0) then
        NFE_001NFE_PSLQ.Value := NFE_001NFE_QCOM.AsCurrency / NFE_001NFE_REND.AsFloat else
    if (LeftStr(NFE_001NFE_UCOM.AsString,1) = 'M') and (NFE_001NFE_METR.AsFloat > 0) and (NFE_001NFE_PESO.AsFloat > 0) then
        NFE_001NFE_PSLQ.Value := (NFE_001NFE_QCOM.AsCurrency * NFE_001NFE_PESO.AsFloat) / NFE_001NFE_METR.AsFloat else
    if  NFE_001NFE_PESO.AsFloat > 0 then
        NFE_001NFE_PSLQ.Value := (NFE_001NFE_PESO.AsFloat * NFE_001NFE_QCOM.AsCurrency) / IFThen(NFE_001NFE_QTUN.AsFloat > 0,NFE_001NFE_QTUN.AsFloat,1);
...
begin
  ...
  begin
    ...
    oRefresh(NFE_001,True,'',NFE_001.GetBookmark);
  end;
end;

procedure Tfrmven_nfe.NFE_001UpdateError(DataSet: TDataSet;
...
procedure Tfrmven_nfe.NFE_001EditError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure Tfrmven_nfe.NFE_001DeleteError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure Tfrmven_nfe.NFE_001PostError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure Tfrmven_nfe.NFE_001CalcFields(DataSet: TDataSet);
begin
  NFE_001C_ID.Value := NFE_001.RecNo;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AColumn = DBGEdicaoNFE_VDESC) and (ANode.Values[DBGEdicaoNFE_VDESC.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_VDESC.Index] > 0) then
      begin
        AFont.Style := [fsBold];
      end;

      if (AColumn = DBGEdicaoNFE_VICMS) and (ANode.Values[DBGEdicaoNFE_VICMS.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_VICMS.Index] > 0) then
      begin
        AColor      := clInfoBk;
        AFont.Color := clBlack;
        AFont.Style := [fsBold];
      ...
      if (AColumn = DBGEdicaoNFE_PREDBC) and (ANode.Values[DBGEdicaoNFE_PREDBC.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_PREDBC.Index] > 0) then
      begin
        AColor      := clBtnFace;
        AFont.Color := clBlack;
      end;

      if (AColumn = DBGEdicaoNFE_VICMSST) and (ANode.Values[DBGEdicaoNFE_VICMSST.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_VICMSST.Index] > 0) then
      begin
        AColor      := clInfoBk;
        AFont.Color := clBlack;
        AFont.Style := [fsBold];
      ...
      if (ANode.Values[DBGEdicaoNFE_VIPI.Index] > 0) then
      begin
        AColor      := clInfoBk;
        AFont.Color := clBlack;
        AFont.Style := [fsBold];
      ...
      if (AColumn = DBGEdicaoNFE_EXTIPI) and (ANode.Values[DBGEdicaoNFE_EXTIPI.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_EXTIPI.Index] <> EmptyStr) then
      begin
        AColor      := clBtnFace;
        AFont.Color := clBlack;
      end;

      if (AColumn = DBGEdicaoNFE_CEST) and (ANode.Values[DBGEdicaoNFE_CEST.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_CEST.Index] > 0) then
      begin
        AColor      := clBtnFace;
        AFont.Color := clBlack;
      end;
      ...
      if (AColumn = DBGEdicaoNFE_VPIS) and (ANode.Values[DBGEdicaoNFE_VPIS.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_VPIS.Index] > 0) then
      begin
        AColor      := clInfoBk;
        AFont.Color := clBlack;
      end;

      if (AColumn = DBGEdicaoNFE_VCOFINS) and (ANode.Values[DBGEdicaoNFE_VCOFINS.Index] <> Null) then
      if (ANode.Values[DBGEdicaoNFE_VCOFINS.Index] > 0) then
      begin
        AColor      := clInfoBk;
        AFont.Color := clBlack;
      end;
...
begin
  case key of
    vk_return: if NFE_001.State in [dsInsert,dsEdit] then
               begin
  ...
  try
    ...
    if CECDRO.Value > 0 then
       ...
       begin
         ...
         oException(IEBOLETO,'Nota Fiscal sem Duplicata(s) Registrada(s) !');
       end;
...
begin
  ...
  if CECDRO.Value > 0 then
  begin
    if (Pos(LeftStr(RECRomaneio.STCO,3),'BANBOLDUP') > 0) and ((DUP_001ID.AsInteger = 0) or (IEBOLETO.Text <> 'SIM')) then
  ...
  try
    ...
    begin
      ...
      begin
        if NFE_001NFE_QCOM.AsCurrency = 0 then
        begin
    ...
    if (BCALC) then
    if (LeftStr(PECFOP.Text,1) = '3') or (LeftStr(PECFOP.Text,1) = '7') then
    oRTransact(TSEdicao);
  end;
end;

procedure Tfrmven_nfe.ACTPesquisasExecute(Sender: TObject);
begin
  { INICIALIZA FORM SCREEN }
  Screen.Cursor := crHourGlass;
end;

procedure Tfrmven_nfe.ACTCheckNFeSeqExecute(Sender: TObject);
begin
  AID  := 0;
  ADIF := 0;
...
begin
  if TAB_CFOPCFOP.AsString <> NFE_001NFE_CFOP.AsString then
  begin
    TAB_CFOP.Close;
    TAB_CFOP.ParamByName('EP_ID').Value := RECParametros.EP_ID;
    TAB_CFOP.ParamByName('CFOP' ).Value := NFE_001NFE_CFOP.AsString;
    TAB_CFOP.Prepare;
    TAB_CFOP.Open;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      NFE_001NFE_QTRIB.Value   := NFE_001NFE_QCOM.AsCurrency;
      NFE_001NFE_VUNTRIB.Value := NFE_001NFE_VUNCOM.AsFloat;
    end;

    NFE_001NFE_VPROD.Value := NFE_001NFE_QCOM.AsCurrency * NFE_001NFE_VUNCOM.AsFloat;
  end;
end;
...
begin
  ...
  NFE_001NFE_CSTIPI.Value := TAB_CFOPCFOP_CSTIPI.AsString;
end;

procedure Tfrmven_nfe.NFE_001NFE_VPRODValidate(Sender: TField);
begin
  if (BCALC) and (NFE_001NFE_QTRIB.AsCurrency > 0) and (NFE_001NFE_VPROD.AsFloat > 0) then
  NFE_001NFE_VUNTRIB.Value := NFE_001NFE_VPROD.AsFloat / NFE_001NFE_QTRIB.AsCurrency;
end;
...
begin
  ...
  if ((NFE_001NFE_CSTIPI.AsString = '04') and (IECFOP_TPNF.Text = '0')) or ((NFE_001NFE_CSTIPI.AsString = '54') and (IECFOP_TPNF.Text = '1')) then
  ...
  if ((NFE_001NFE_CSTIPI.AsString = '05') and (IECFOP_TPNF.Text = '0')) or ((NFE_001NFE_CSTIPI.AsString = '55') and (IECFOP_TPNF.Text = '1')) then
  ...
  if ((NFE_001NFE_CSTIPI.AsString = '02') and (IECFOP_TPNF.Text = '0')) or ((NFE_001NFE_CSTIPI.AsString = '52') and (IECFOP_TPNF.Text = '1')) then
  ...
  if ((NFE_001NFE_CSTIPI.AsString = '49') and (IECFOP_TPNF.Text = '0')) or ((NFE_001NFE_CSTIPI.AsString = '99') and (IECFOP_TPNF.Text = '1')) then
...
procedure Tfrmven_nfe.ACTPSQNotaFiscalExecute(Sender: TObject);
begin
  { VER REGISTROS }
  ...
  begin
    ...
    SQL.Add('       FK.IDCP   ,FK.NFE_CPROD,FK.NFE_CEAN  ,');
    SQL.Add('       FK.NFE_NCM,FK.NFE_PIPI ,FK.NFE_EXTIPI,FK.NFE_CEST,');
  ...
  begin
    ...
    begin
      ...
      nfe_001NFE_UTRIB.Value  := SQLPKConsulta.Current.ByName('NFE_UTRIB').AsString;
      ...
      nfe_001NFE_QCOM.Value   := SQLPKConsulta.Current.ByName('NFE_QCOM').AsCurrency;
...
procedure Tfrmven_nfe.ACTPSQRomaneioExecute(Sender: TObject);
begin
  try
    { INICIALIZA FORM SCREEN }
    Screen.Cursor     := crHourGlass;
    ...
    PESQUISA_NATUREZA(SQLPKConsulta.Current.ByName('CFOP').AsString,'C'); { Transportadora }
    ...
    V1_CEQVOL.Text := SQLPKConsulta.Current.ByName('QTRL').AsString;
    ...
    begin
      ...
      nfe_001.Append;
      nfe_001NFE_ITEMPED.Value := SQLPKConsulta.Current.ByName('ITEM' ).AsInteger;
      nfe_001NFE_IPRO.Value    := SQLPKConsulta.Current.ByName('CP_ID').AsInteger;
      nfe_001NFE_CPROD.Value   := SQLPKConsulta.Current.ByName('SKU'  ).AsString;
      nfe_001NFE_CEAN.Value    := SQLPKConsulta.Current.ByName('CEAN' ).AsString;

      nfe_001NFE_XPROD.Value   := SQLPKConsulta.Current.ByName('CP_NO'    ).AsString;
      nfe_001NFE_COMP.Value    := SQLPKConsulta.Current.ByName('CP_NO_CMP').AsString;

      nfe_001NFE_UCOM.Value    := SQLPKConsulta.Current.ByName('UCOM' ).AsString;
      nfe_001NFE_UTRIB.Value   := SQLPKConsulta.Current.ByName('UTRIB').AsString;

      nfe_001NFE_NCM.Value     := SQLPKConsulta.Current.ByName('NCM').AsString;
      nfe_001NFE_PIPI.Value    := SQLPKConsulta.Current.ByName('PIPI').AsFloat;
      nfe_001NFE_CEST.Value    := SQLPKConsulta.Current.ByName('CEST').AsString;
      nfe_001NFE_EXTIPI.Value  := SQLPKConsulta.Current.ByName('EXTIPI').AsString;

      nfe_001NFE_QTUN.Value    := SQLPKConsulta.Current.ByName('UQTDE').AsFloat;
      nfe_001NFE_PSCN.Value    := SQLPKConsulta.Current.ByName('UPSCN').AsFloat;

      nfe_001NFE_PESO.Value    := SQLPKConsulta.Current.ByName('MPESO' ).AsFloat;
      nfe_001NFE_METR.Value    := SQLPKConsulta.Current.ByName('MMETRO').AsFloat;
      nfe_001NFE_REND.Value    := SQLPKConsulta.Current.ByName('MREND' ).AsFloat;

      nfe_001NFE_QCOM.Value    := SQLPKConsulta.Current.ByName('QTDE').AsCurrency;
      nfe_001NFE_RCOM.Value    := SQLPKConsulta.Current.ByName('QTRL').AsInteger;

      nfe_001NFE_PREC.Value    := SQLPKConsulta.Current.ByName('VPRC_COM').Value;
      nfe_001NFE_VDESC.Value   := SQLPKConsulta.Current.ByName('PK_VDSC' ).Value;
      nfe_001NFE_VUNCOM.Value  := SQLPKConsulta.Current.ByName('VPRC_COM').Value;

      nfe_001NFE_VPROD.Value   := SQLPKConsulta.Current.ByName('TCDE').Value;

      nfe_001NFE_STAV.Value    := SQLPKConsulta.Current.ByName('REST').AsString;
      nfe_001NFE_ORIG.Value    := SQLPKConsulta.Current.ByName('ORIG').AsString;
      nfe_001NFE_NFCI.Value    := SQLPKConsulta.Current.ByName('NFCI').AsString;

      { INFORMAÇÕES ADICIONAIS }
      nfe_001NFE_INFADPROD.Value := SQLPKConsulta.Current.ByName('CP_NO_CMP').AsString;

      if not oEmpty(nfe_001NFE_INFADPROD.AsString) then
      nfe_001NFE_INFADPROD.Value := 'Composição Têxtil: ' + nfe_001NFE_INFADPROD.AsString;

      if ((SQLPKConsulta.Current.ByName('UQVOL').AsFloat > 0) and (not oEmpty(SQLPKConsulta.Current.ByName('UESP').AsString))) then
      nfe_001NFE_INFADPROD.Value := nfe_001NFE_INFADPROD.AsString + #13 + 'Conteúdo Unitário: ' + SQLPKConsulta.Current.ByName('UQVOL').AsString + ' ' + SQLPKConsulta.Current.ByName('UESP').AsString else

      if Pos(LeftStr(nfe_001NFE_UCOM.AsString,1),'KMY') = 0 then
      if nfe_001NFE_METR.AsFloat > 0 then
      nfe_001NFE_INFADPROD.Value := nfe_001NFE_INFADPROD.AsString + #13 + 'Conteúdo Unitário: ' + FormatFloat('0.00M',nfe_001NFE_METR.AsFloat);
...
procedure Tfrmven_nfe.ACTPSQLoteExecute(Sender: TObject);
var
  y: Word;
begin
  y := 0;
...
begin
  ...
  PCEdicao.ActivePage := TSProduto;
  if ACTXMLCreate.ImageIndex = 10 then
  begin
    EDITAR;
    Exit;
  end;
...
//  tIde[01] := IntToStr(Random(CECDNF.Value)+1); //<cNF> Código Numérico que compõe a chNFe de Acesso - Numero aleatorio: apenas 8 digitos: NF-e 2.00
    ...
    tIde[02] := oREPAcentos(PECFOP_NO.Text); //<natOp>  Descrição da Natureza da Operação
    ...
    begin
      ...
      tProd[x,007] := oTextToValor(NFE_001NFE_QCOM.AsCurrency,2,True);                                                  //<qCom> Quantidade Comercial
      tProd[x,007] := oTextToValor(NFE_001NFE_QCOM.AsCurrency,2,True);                                                  //<qCom> Quantidade Comercial
      ...
      tProd[x,012] := oTextToValor(NFE_001NFE_QTRIB.AsCurrency,2,True);                                                 //<qTrib#,0.00> Quantidade Tributável
      ...
      tProd[x,060] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_NDI.AsString+';');                                    //     <nDI>         Número do Documento de Importação (DI, DSI, DIRE, ...)
      tProd[x,061] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',FormatDateTime('yyyy-mm-dd',NFE_001NFE_DDI.AsDateTime)+';');     //     <dDI>         Data de RegEdit do documento - Formato: “AAAA-MM-DD”
      tProd[x,062] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_XLOCDESEMB.AsString+';');                             //     <xLocDesemb>  Local de desembaraço
      tProd[x,063] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_UFDESEMB.AsString+';');                               //     <UFDesemb>    Sigla da UF onde ocorreu o Desembaraço Aduaneiro
      tProd[x,064] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',FormatDateTime('yyyy-mm-dd',NFE_001NFE_DDESEMB.AsDateTime)+';'); //     <dDesemb>     Data do Desembaraço Aduaneiro
      tProd[x,065] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_CEXPORTADOR.Value+';');                               //     <cExportador> Código do Exportador
      tProd[x,066] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_NADICAO.AsString +';');                               //adi: <nAdicao>     Numero da Adição (1-1)
      tProd[x,067] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_NSEQADIC.AsString+';');                               //adi: <nSeqAdic>    Numero sequencial do item dentro da Adição (1-1)
      tProd[x,068] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_CFABRICANTE.AsString+';');                            //adi: <cFabricante> Código do fabricante estrangeiro (1-1)
      ...
      tProd[x,155] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_NDRAW.AsString+';');                                  //adi: <nDraw>        Número do ato concessório de Drawback
      tProd[x,106] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_TPVIATRANSP.AsString +';');                           //     <tpViaTransp>  NT 2013.005 1=Marítima; 2=Fluvial; 3=Lacustre; 4=Aérea; 5=Postal 6=Ferroviária; 7=Rodoviária;  8=Conduto / Rede Transmissão; 9=Meios Próprios; 10=Entrada / Saída ficta
      tProd[x,107] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_TPINTERMEDIO.AsString+';');                           //     <tpIntermedio> 1=Importação por conta própria; 2=Importação por conta e ordem; 3=Importação por encomenda;
      tprod[x,133] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',oTextToValor(NFE_001NFE_VAFRMM.AsFloat,2,True)+';');             //     <vAFRMM>       Valor da AFRMM - Adicional ao Frete para Renovação da Marinha Mercante
      tprod[x,134] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_CNPJTERCEIRO.AsString+';');                           //     <CNPJ>         CNPJ do adquirente ou do encomendante
      tprod[x,135] := IFThen(LeftStr(PECFOP.Text,1) <> '3','',NFE_001NFE_UFTERCEIRO.AsString+';');                             //     <UFTerceiro>   Sigla da UF do adquirente ou do encomendante
    ...
    tPag[01] := IFThen(IETPAG.Text   = '90','0.00;',oTextToValor(IFThen(IEBOLETO.Text = 'SIM',EDVDup.Text,EDVNF.Text),2,True)+';'); //<vPag> Valor do Pagamento
...
procedure Tfrmven_nfe.ACTXMLLoteExecute(Sender: TObject);
var
  x,y : Integer;
begin
  y := 0;
  ...
  try
    ...
    try
      ...
      begin
        ...
        SPEdicao.ParamByName('QCOM').Value           := NFE_001NFE_QCOM.AsCurrency;
...
procedure Tfrmven_nfe.ACTNFeUPDExecute(Sender: TObject);
begin
  try
    oOTransact(TEdicao);
    with SQLEdicao do
...
procedure Tfrmven_nfe.ACTCAD_PAD_EDTExecute(Sender: TObject);
begin
  try
    with SQLEdicao do
    begin
...
procedure Tfrmven_nfe.ACTCAD_PRO_EST_LANExecute(Sender: TObject);
var
  REC_SHE_DEF: TREC_SHE_DEF;
begin
  if LeftStr(PECFOP.Text,1) = '3' then
...
begin
  ACTXMLCreate.Enabled    := True;
  ACTXMLCreate.ImageIndex := 0;
  ACTXMLCreate.Caption    := 'Gerar e Assinar';

  ACTXMLValidate.Enabled := False;
  ACTXMLSefaz.Enabled    := False;

  ACTNFePrint.Enabled := false;
  ACTNFeEmail.Enabled := false;
...
begin
  ...
  ACTXMLCreate.ImageIndex := 10;
  ACTXMLCreate.Caption    := 'Editar';
  ...
  try
    ...
    try
      ...
      begin
        ...
        AINFADCAD.Add('NF de Retorno ('+RightStr(PECFOP.Text,4)+') Ref. a(s) NF(s).: |');
...
begin
  if (not ALockWindowUpdate) and (BCALC) then
  DUPLICATAS;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      PESQUISA_NATUREZA(PECFOP.Text,'C');
    end;
...
procedure Tfrmven_nfe.PECFOPChange(Sender: TObject);
begin
  if LeftStr(PECFOP.Text,1) = '3' then
  begin
...
//  if not ALockWindowUpdate then CALCULA_NF;
end;
...
procedure Tfrmven_nfe.EDDECDChange(Sender: TObject);
begin
  EDDECD.Hint := EDDECD.Text;
end;
...
procedure Tfrmven_nfe.PECFOP_NOChange(Sender: TObject);
begin
  PECFOP_NO.Hint := PECFOP_NO.Text;
end;
...
procedure Tfrmven_nfe.PEDECTChange(Sender: TObject);
begin
  PEDECT.Hint := PEDECT.Text;
end;
...
begin
  ...
  if Pos(PECFOP.Text,'59246924') > 0 then
  begin
    EDINFADNF.Lines.Clear;
  ...
  if (Length(PECFOP.Text) = 4) and (Pos(PECFOP.Text,'5123612359246924') > 0) then
  begin
    PCEdicao.ActivePage := TSNFTriangular;
    ATUALIZA_MENSAGENS;
  end;
...
begin
  ...
  try
    ...
    if (BCalc) and (IEBOLETO.Text = 'SIM') and (IECDPG.Text <> EmptyStr) then
    begin
      TAB_PAG.Close;
...
procedure Tfrmven_nfe.ACTXMLSefazExecute(Sender: TObject);
begin
  ACTXMLLote.Execute; { Gera e Envia Lote }
  ACTPSQLote.Execute; { Pesquisa Lote Retornado }
  ...
  begin
    ACTNFePost.Execute ; { Salva Nota Fiscal }
    ACTNFeDanfe.Execute; { Gerar e Imprimir Danfe }

    ACTXMLCreate.Enabled   := False;
    ACTXMLValidate.Enabled := False;
    ACTXMLSefaz.Enabled    := False;

    ACTNFePrint.Enabled := True;
    ACTNFeEmail.Enabled := True;
    ACTImporta.Enabled  := False;
  end;
...
begin
  ...
  begin
    NFE_001.Edit;
    if (RoundTO(NFE_001NFE_VFRETE.AsFloat,-2)*i) > (CEVFRT.Value) then
    NFE_001NFE_VFRETE.Value := NFE_001NFE_VFRETE.AsFloat - ((RoundTO(NFE_001NFE_VFRETE.AsFloat,-2)*i)-CEVFRT.Value) else
...
begin
  ...
  begin
    NFE_001.Edit;
    if (RoundTO(NFE_001NFE_VSEG.AsFloat,-2)*i) > (CEVSEG.Value) then
    NFE_001NFE_VSEG.Value := NFE_001NFE_VSEG.AsFloat - ((RoundTO(NFE_001NFE_VSEG.AsFloat,-2)*i)-CEVSEG.Value) else
...
begin
  ...
  begin
    NFE_001NFE_PREDBC.Value := TAB_CFOPCFOP_PREDBC.AsFloat;
    NFE_001NFE_PICMS.Value  := IFThen((Pos(NFE_001NFE_CST.AsString,'201202500900') = 0) and (RECParametros.NFE_CRT = 1),0,
                               ...
                               IFThen((PEUF.Text = RECParametros.LOG_UF      ) and (PEUF.Text = 'SP') and
                                      (NFE_001NFE_NCM.AsString   = '39181000'),12,tab_alqALQ_ICMS.AsFloat))));
...
begin
  if (IEFinNFe.Text = '2') or { Complementar }
     (not BCALC) then
  Exit;

  { ICMS }
  NFE_001NFE_CST.Value := TAB_CFOPCFOP_CSTICMS.AsString;

  { IPI }
  NFE_001NFE_CSTIPI.Value := TAB_CFOPCFOP_CSTIPI.AsString;

  { PIS }
  NFE_001NFE_CSTPIS.Value := TAB_CFOPCFOP_CSTPIS.AsString;
  NFE_001NFE_PPIS.Value   := TAB_CFOPCFOP_PPIS.AsFloat;

  { COFINS }
  NFE_001NFE_CSTCOFINS.Value := TAB_CFOPCFOP_CSTCOFINS.AsString;
  NFE_001NFE_PCOFINS.Value   := TAB_CFOPCFOP_PCOFINS.AsFloat;

  NFE_001NFE_INDTOT.Value := TAB_CFOPCFOP_INDTOT.AsInteger;
  ...
  begin
    ...
    NFE_001NFE_CST.Value := TAB_CFOPCFOP_CST_CSOSNCRED.AsString; { Crédito ICMS }
  end;
end;
...
begin
  if ALockWindowUpdate then { Edição em Massa }
  Exit;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        { RATEIOS }
        NFE_001NFE_VDESC.Value  := IFThen(ADSC_NREG > NFE_001.RecNo,ADSC_VREG,ADSC_VSLD); { Descontos }
        NFE_001NFE_VFRETE.Value := IFThen(AFRT_NREG > NFE_001.RecNo,AFRT_VREG,AFRT_VSLD); { Frete }
        NFE_001NFE_VSEG.Value   := IFThen(ASEG_NREG > NFE_001.RecNo,ASEG_VREG,ASEG_VSLD); { Seguro }
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                NFE_001NFE_CFOP.Value    := IFThen(IECFOP_TPNF.Text = '0','2411',
                ...
                if (NFE_001NFE_CFOP.AsString = '6404') and (PECFOP.Text = '6403') then
                    NFE_001NFE_CFOP.Value := PECFOP.Text;
              ...
              if RECParametros.NFE_REDUCAO_PISCOFINS then
                 if ((IECFOP_TPNF.Text = '1') or (Pos(PECFOP.Text,'1201120222012202') > 0)) and (NFE_001NFE_VBCPIS.AsFloat > 0) and (NFE_001NFE_VBCOFINS.AsFloat > 0) and (NFE_001NFE_VICMS.AsFloat > 0) then
                 begin
                   { REDUÇÃO PIS}
...
begin
  if (Pesq = EmptyStr) or (Chave = EmptyStr) then
  Exit;

  if PESQ <> TAB_CFOPCFOP.AsString then
  begin
    TAB_CFOP.Close;
    TAB_CFOP.ParamByName('EP_ID').Value := RECParametros.EP_ID;
    TAB_CFOP.ParamByName('CFOP' ).Value := PESQ;
    TAB_CFOP.Prepare;
    TAB_CFOP.Open;
  end;
  ...
  begin
    ...
    begin
      NFE_001.Edit;
      NFE_001NFE_CFOP.Value := TAB_CFOPCFOP.AsString;
      NFE_001.Post;
    end;
  ...
  begin
    ...
    ACTNFeUPD.HelpKeyword := 'TRIANGULAR';
    ACTNFeUPD.Execute;
  end;
end;

procedure Tfrmven_nfe.ACTNFeEmailExecute(Sender: TObject);
begin
       ...
       try
         ...
         begin
           ...
           begin
             ...
             NFE_001NFE_QCOM.Value     := NFeNFE_QCOM.AsCurrency;
         ...
         begin
           ...
           PECFOP.Text := imp_nitNFE_CFOP.AsString;
           PESQUISA_NATUREZA(PECFOP.Text,'C');
           ...
           if XMLModFrete <> EmptyStr then
              IEModFrete.Text := IFThen(LeftStr(PECFOP.Text,1) = '3',XMLModFrete,'4') else
           ...
           if NFE_001NFE_CCAB.AsInteger = 99999 then
              ...
              begin
                ...
                NFE_001NFE_VUNCOM.AsFloat := IFThen(NFE_001NFE_VUNCOM.AsFloat <> NFE_001NFE_VPROD.AsFloat / NFE_001NFE_QCOM.AsCurrency,NFE_001NFE_VPROD.AsFloat / NFE_001NFE_QCOM.AsCurrency,NFE_001NFE_VUNCOM.AsFloat);
                NFE_001.Post;
                NFE_001.Next;
              end;
         end else
...
procedure Tfrmven_nfe.ACTNFePrintExecute(Sender: TObject);
begin
  if (FileExists(NFE_PDF)) and (FileExists(NFE_XML)) then
      try

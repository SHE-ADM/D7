...
type
  Tfrmven_nfe = class(TForm)
    ...
    procedure CEQVOLChange(Sender: TObject);
...
begin
  ...
  begin
    ...
    { Descrição XML }
    PNLXPROX.Caption :=  EdicaoCP_CMP_NO.AsString;
    PNLXPROX.Visible := (EdicaoCP_CMP_NO.AsString <> EmptyStr);
...
begin
  if not ASelected then
     if ANode.Values[DBGEdicaoFLAG_CTRL.Index] = 0 then
     begin
         AColor      := clGrayText;
         AFont.Color := clHighLightText;
     end else
     begin
       if (AColumn = DBGEdicaoNFE_NITEMPED) or (AColumn = DBGEdicaoNFE_XPED) or (AColumn = DBGEdicaoNFE_CPROD) then
       begin
         AColor      := clBtnFace;
         AFont.Color := clBlack;
       end;
...
begin
  ...
  begin
    try
      ...
      SBRodape.Panels[1].Text :=  NfeStatusServico;
      SBRodape.Refresh;

      if (SBRodape.Panels[1].Text = EmptyStr) or (LeftStr(SBRodape.Panels[1].Text,3) <> '107') then
      begin
        SBRodape.Panels[0].Text := 'Falha na conexão com o SEFAZ';
        SBRodape.Panels[1].Text := 'Verifique certificado, internet ou disponibilidade do SEFAZ em https://www.nfe.fazenda.gov.br/portal/disponibilidade.aspx';
        SBRodape.Refresh;

        Abort;
      end else
      begin
        SBRodape.Panels[0].Text := 'Conexão estabelecida com sucesso';
        SBRodape.Panels[1].Text := 'SEFAZ ON LINE';
        SBRodape.Refresh;
      end;
    ...
    begin
      if IECDPG.Text <> TAB_PAGPAG_DPAG.AsString then
      begin
        TAB_PAG.Close;
    ...
    { Consumidor Final }
    if (RECParametros.NFE_CRT = 3) and (IEIndFinal.Text = '1') and (IECFOP_TPNF.Text = '1') and (PEUF.Text <> 'EX') and (PEUF.Text <> RECParametros.LOG_UF) then
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            sWarning := sWarning +#13+'Item: '+EdicaoNFE_NITEMPED.AsString;
            Break;
          end;
          ...
          if IEFinNFe.Text <> '2' then
             ...
             begin
               ...
               oException(DBGEdicao,'NCM não Informado ou Incorreto !'+#13+#13+
                                 'Item '+EdicaoNFE_NITEMPED.AsString+' - '+EdicaoNFE_CPROD.AsString+'.');
             end;
...
begin
  ...
  { Assina }
  SBRodape.Panels[0].Text := 'Aguarde, assinando arquivo XML ...';
  SBRodape.Panels[1].Text :=  EmptyStr;
  SBRodape.Refresh;
  SleepEx(500,False);

  SBRodape.Panels[0].Text := 'Pronto';
  SBRodape.Panels[1].Text :=  AssinaArquivoXML(PChar('C:\Sheild\NotaFiscal\NFe\arquivos\'+chNFe+'.xml'),'infNFe');
  SBRodape.Refresh;
  SleepEx(500,False);

  BRet := (LeftStr(SBRodape.Panels[1].Text,1) = 'A');
  if not BRet then
     SBRodape.Panels[0].Text := 'Falha na assinatura';
     SBRodape.Refresh;

  { Valida }
  if BRet then
  begin
    SBRodape.Panels[0].Text := 'Aguarde, validando assinatura ...';
    SBRodape.Panels[1].Text := EmptyStr;
    SBRodape.Refresh;
    ...
    if BRet then
    begin
      SBRodape.Panels[0].Text := 'Validação efetuada com sucesso';
      SBRodape.Panels[1].Text := 'Nenhum erro identificado';
    end else
    begin
      SBRodape.Panels[0].Text := 'Falha na validação';
      SBRodape.Panels[1].Text := 'Problemas na validação da assinatura do arquivo XML. Verifique os erros encontrados';
    end;
...
begin
  ...
  SBRodape.Panels[0].Text := 'Aguarde, Transmitindo arquivo XML ...';
  SBRodape.Panels[1].Text := NfeRetAutorizacao(protNFe);
  SBRodape.Refresh;
  ...
  try
    ...
    SBRodape.Panels[0].Text := 'Pronto';
    SBRodape.Panels[1].Text := Trim(FrmNFeConsultaSefaz.IEdStat.Descriptions[FrmNFeConsultaSefaz.IEdStat.Values.IndexOf(FrmNFeConsultaSefaz.IEdStat.Text)]) + #13 + #13 +
                             Trim(FrmNFeConsultaSefaz.EMConsulta.Text);
    SBRodape.Panels[2].Text := chNFe;
    SBRodape.Panels[3].Text := protNFe;
    SBRodape.Refresh;
  ...
  begin
    SBRodape.Panels[0].Text := 'Falha';
    SBRodape.Refresh;

    Editar;
    oException(Nil,'Falha na autorização da emissão !' + #13 +
                   'Erro: ' + SBRodape.Panels[1].Text);
  end;

  try
    SBRodape.Panels[0].Text := 'Aguarde, registrando NFe ...';
    SBRodape.Refresh;

    SALVA_NF;

    SBRodape.Panels[0].Text := 'Pronto';
    SBRodape.Panels[1].Text := 'NFe registrada com sucesso';
    SBRodape.Refresh;
  except
    SBRodape.Panels[0].Text := 'Falha no registro da NFe';
    SBRodape.Refresh;
  end;
  ...
  try
    ...
    SBRodape.Panels[0].Text := 'Pronto';
    SBRodape.Panels[1].Text := 'Danfe gerada com sucesso';
    SBRodape.Refresh;
  except
    SBRodape.Panels[0].Text := 'Falha na geração da danfe';
    SBRodape.Refresh;
  end;
...
begin
  ...
  begin
    ...
    begin
      Edicao.Edit;
      EdicaoNFE_CFOP.Value := TAB_CFOPCFOP.AsString;
      Edicao.Post;
    end;
...
begin
  try
    ...
    SBRodape.Panels[0].Text := 'Aguarde, gerando lote de envio ...';
    SBRodape.Panels[1].Text :=  EmptyStr;
    SBRodape.Panels[3].Text :=  EmptyStr;
    SBRodape.Refresh;
    ...
    begin
      SBRodape.Panels[0].Text := 'Falha na geração do lote';
      if x = -1 then
         SBRodape.Panels[1].Text := 'Falha, Erro Desconhecido !' else
      if x = 0 then
         SBRodape.Panels[1].Text := 'Falha, Operação Cancelada !' else
      if x = 2 then
         SBRodape.Panels[1].Text := 'Falha, Não existe(m) arquivo(s) assinado(s) !' else
         SBRodape.Panels[1].Text := 'Falha na geração do lote !';
         SBRodape.Refresh;
    end else
    begin
      SBRodape.Panels[0].Text := 'Lote gerado com sucesso';
      SBRodape.Panels[1].Text := 'Enviando SEFAZ ...';
      SBRodape.Refresh;
      SleepEx(1000,False);
      ...
      SleepEx(1000,False);
      if (Pos('103',SBRodape.Panels[1].Text) = 0) or (Trim(RightStr(SBRodape.Panels[1].Text,Length(SBRodape.Panels[1].Text) - Pos('#',SBRodape.Panels[1].Text))) = EmptyStr) then
      begin
        BRet := False; EDITAR;
        oException(Nil,'Falha na autorização da assinatura !'+#13+
                    ...
                    protNFe := Trim(RightStr(SBRodape.Panels[1].Text,Length(SBRodape.Panels[1].Text) - Pos('#',SBRodape.Panels[1].Text)));
      SBRodape.Panels[3].Text := Trim(RightStr(SBRodape.Panels[1].Text,Length(SBRodape.Panels[1].Text) - Pos('#',SBRodape.Panels[1].Text)));
      SBRodape.Refresh;
    end;
  finally
    Screen.cursor := crDefault;
...
begin
  ...
  try
    ...
    begin
      AINFADCAD.Add(Trim('Permite o aproveitamento do credito de ICMS no valor de '+FormatFloat('R$ #,0.00',FIS_NFE_SUMNFE_VCREDICMSSN.AsFloat))            +'|');
    ...
    begin
      ...
      begin
        ...
        AINFADCAD.Add('ICMS Isento conforme Artigo no 84 do Anexo I do RICMS/SP. Desconto de 7% - ICMS: ' + FormatFloat('R$ #,0.00',FIS_NFE_SUMNFE_VICMSDESON.AsFloat) + '|');
      end;
    end;
    ...
    begin
      ...
      AINFADCAD.Add(Trim('Decreto no 62.560/2017 (DOE de 06.05.2017) = '+FormatFloat('#,0.00%',EdicaoNFE_PREDBC.AsFloat))+'|');
    end;

    if CLI_001.RecNo > 0 then
    begin
      CLI_001.First;
      if CLI_001.RecNo > 0 then AINFADCAD.Add('|');
    ...
    begin
      AINFADCAD.Add('Valor Total ICMS Interestadual UF Destino: '+FormatFloat('Valor de R$ #,0.00',FIS_NFE_SUMNFE_VICMSUFDEST.AsFloat));
      AINFADCAD.Add('|');
    end;
    ...
    if TXTCFOP <> EmptyStr then AINFADCAD.Add('|');

    While Length(TXTCFOP) > 0 do
    begin
      TXTInfAd := oREPAcentos(Fetch(TXTCFOP,#$D#$A));
      if Length(TXTInfAd) > 0 then AINFADCAD.Add(TXTInfAd+'|');
    end;
  finally
    AINFADCAD.EndUpdate;
...
begin
  ...
  EdicaoCP_ID.Value   := FBird.FBCAD_PROId.AsInteger;
  ...
  EdicaoCP_UQTDE.Value := FBird.FBCAD_PROUQTDE.AsInteger; { RICARDO }

  EdicaoCP_CMP_NO.Value      := Trim(FBird.FBCAD_PROCMP_PAD.AsString);
  ...
  try
    ...
    try
      ...
      begin
        ...
        SPEdicao.ParamByName('DCOR').Value           := '';
        ...
        SPEdicao.ParamByName('VIOFIMP').Value        := EdicaoNFE_VIOF.AsFloat;
        ...
        SPEdicao.ParamByName('CMUNFGISSQN').Value    := '';
        SPEdicao.ParamByName('CLISTSERV').Value      := EdicaoNFE_CLISTSERV.AsString;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT COUNT(*) FROM FIS_NFE_ITE');
    ExecQuery;
...
begin
  ...
  begin
    ...
    begin
      ...
      Close;
      SQL.Clear;
      SQL.Add('DELETE FROM FIS_NFE_ITE WHERE NFE_PIPI = 0');
      ExecQuery;
    end;
    oRefresh(Edicao);
  ...
  if LeftStr(PECFOP.Text,1) = '3' then
     try
       ...
       begin
         ...
         SPEdicao.ParamByName('INFADTEC').Value := EdicaoCP_CMP_NO.AsString;
...
procedure Tfrmven_nfe.CEQVOLChange(Sender: TObject);
begin
  if TdxCurrencyEdit(Sender).TAG > 1 then
  begin
    tdxCurrencyEdit(FindComponent(Format('V%d_CENVOL',[TdxCurrencyEdit(Sender).TAG]))).Value := CENVOL.Value;
    tdxEdit(FindComponent(Format('V%d_EDMARCA',[TdxEdit(Sender).TAG]))).Text := EDMARCA.Text;
  end;
end;
...
begin
  ...
  try
    ...
    i := 0;
    repeat
      SBRodape.Panels[1].Text := ArquivoDistribuicaoNFe(RightStr(chNFe,44));
      SBRodape.Refresh;
...
var
  ...
  BRet: Boolean;
begin
  SBRodape.Panels[0].Text := 'Aguarde, gerando arquivo XML ...';
  SBRodape.Panels[1].Text :=  EmptyStr;
  SBRodape.Refresh;
  SleepEx(1000,False);
...
//  tIde[01] := IntToStr(Random(CECDNF.Value)+1); //<cNF> Código Numérico que compõe a chNFe de Acesso - Numero aleatorio: apenas 8 digitos: NF-e 2.00
    ...
    begin
      ...
      if (not oEmpty(EdicaoCP_CMP_NO.AsString)) or (not oEmpty(oREPMemoEsp(EdicaoNFE_INFADProd.AsString))) then
    ...
    begin
      AINFADCAD.Add('** CONTAINER No ..: ' + EDMARCA.Text + '  ' + CENVOL.Text);
      AINFADCAD.Add('|');
    end;
    ...
    begin
      BRet := True;
      SBRodape.Panels[0].Text := 'Arquivo XML gerado com sucesso';
      SBRodape.Panels[1].Text := 'C:\Sheild\NotaFiscal\NFe\' + chNFe + '.xml';
    end else
    begin
      BRet := False;
      SBRodape.Panels[0].Text := 'Falha na geração do arquivo XML';
      SBRodape.Panels[1].Text := EmptyStr;
    end;
  finally
    Screen.cursor := crDefault;
...
begin
  if (BCALC) and (EdicaoNFE_QTRIB.AsCurrency > 0) and (EdicaoNFE_VPROD.AsFloat > 0) then
...
begin
  try
    ...
    if (FIS_NFE_SUMNFE_VNF.AsFloat <> FIS_NFE_SUMNFE_VDUP.AsFloat) and (BCALC) then
    begin
      with SQLSEdicao do
      begin
        Close;

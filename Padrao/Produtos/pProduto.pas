...
uses uPrincipal, bPrincipal,
  pRelatorios, qFicha_Tecnica, pCAD_PRO_EDI, pRelatorios_OLD;
...
procedure TFrmProduto.DTSCAD_PRO_IMG_CDNDataChange(Sender: TObject;
  Field: TField);
begin
...
begin
  FrmRelatorios_OLD := TFrmRelatorios_OLD.Create(Self,ANome,0);
...
begin
  inherited;
  FrmCAD_PRO_EDI := TFrmCAD_PRO_EDI.Create(Self,ArtigosIDAK.AsInteger);
  FrmCAD_PRO_EDI.Show;
end;
...
begin
  ...
  if CAD_PRO_IMG_CDN.State = dsBrowse then CAD_PRO_IMG_CDN.Close;
...
begin
  ...
  if CAD_PRO_IMG_CDN.State = dsInactive then CAD_PRO_IMG_CDN.Open; { Imagens }

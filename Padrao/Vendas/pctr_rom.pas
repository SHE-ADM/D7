  ...
  {$ELSE}
     ...
     begin
       ...
       begin
         ...
         begin
           Edicao.Append;
           EdicaoNFE_ITEMPED.Value   := oStrZero(SQLFKConsulta.Current.ByName('ITEM').AsInteger,5);
           EdicaoNFE_IPRO.Value      := SQLFKConsulta.Current.ByName('IDCP').AsInteger;
           EdicaoNFE_CPROD.Value     := SQLFKConsulta.Current.ByName('CPROD').AsString;
           EdicaoNFE_XPROD.Value     := SQLFKConsulta.Current.ByName('XPROD').AsString;
           EdicaoNFE_UCOM.Value      := SQLFKConsulta.Current.ByName('UCOM').AsString;
           EdicaoNFE_CEAN.Value      := SQLFKConsulta.Current.ByName('CEAN').AsString;

           if LeftStr(EdicaoNFE_CFOP.AsString,1) = '7' then
           EdicaoNFE_UTRIB.Value := SQLFKConsulta.Current.ByName('UTRIB').AsString;
           EdicaoNFE_ORIG.Value  := SQLFKConsulta.Current.ByName('ORIG' ).AsInteger;

           EdicaoNFE_NCM.Value    := SQLFKConsulta.Current.ByName('NCM').AsString;
           EdicaoNFE_PIPI.Value   := SQLFKConsulta.Current.ByName('PIPI').AsFloat;
           EdicaoNFE_CEST.Value   := SQLFKConsulta.Current.ByName('CEST').AsString;
           EdicaoNFE_EXTIPI.Value := SQLFKConsulta.Current.ByName('EXTIPI').AsString;

           EdicaoNFE_PESO.Value := SQLFKConsulta.Current.ByName('PESO').AsFloat;
           EdicaoNFE_PSCN.Value := SQLFKConsulta.Current.ByName('PSCN').AsFloat;
           EdicaoNFE_METR.Value := SQLFKConsulta.Current.ByName('METRO').AsFloat;
           EdicaoNFE_REND.Value := SQLFKConsulta.Current.ByName('REND').AsFloat;
           EdicaoNFE_UQTDE.Value := SQLFKConsulta.Current.ByName('UQTDE').AsFloat; { RICARDO }

           EdicaoNFE_QCOM.Value := SQLFKConsulta.Current.ByName('ROM_QTDE').AsCurrency;
           EdicaoNFE_RCOM.Value := SQLFKConsulta.Current.ByName('ROM_QTRL').AsInteger;

           EdicaoNFE_VUNPAD.Value := SQLFKConsulta.Current.ByName('VUNCOM').AsFloat;
           EdicaoNFE_VUNCOM.Value := SQLFKConsulta.Current.ByName('VUNCOM').AsFloat;

           EdicaoNFE_VPROD.Value := SQLFKConsulta.Current.ByName('TCDE').AsCurrency;
           EdicaoNFE_VDESC.Value := SQLFKConsulta.Current.ByName('VDSC').Value;

           EdicaoNFE_STAV.Value      := SQLFKConsulta.Current.ByName('REST').AsString;
           EdicaoNFE_COMP.Value      := SQLFKConsulta.Current.ByName('CMP_PAD').AsString;
           EdicaoNFE_INFADPROD.Value := SQLFKConsulta.Current.ByName('CMP_PAD').AsString;
           EdicaoNFE_NFCI.Value      := SQLFKConsulta.Current.ByName('ROM_NFCI').AsString;
           ...
           if ((SQLFKConsulta.Current.ByName('UQVOL').AsFloat > 0) and (not oEmpty(SQLFKConsulta.Current.ByName('UESP').AsString))) then
                EdicaoNFE_INFADPROD.Value := EdicaoNFE_INFADPROD.AsString+#13+'CONTEUDO: '+SQLFKConsulta.Current.ByName('UQVOL').AsString+' '+SQLFKConsulta.Current.ByName('UESP').AsString
           else
                if Pos(LeftStr(EdicaoNFE_UCOM.AsString,1),'KMY') = 0 then
                   if  EdicaoNFE_METR.AsFloat > 0 then
                       EdicaoNFE_INFADPROD.Value := EdicaoNFE_INFADPROD.AsString+#13+'CONTEUDO: '+FormatFloat('0.00M',EdicaoNFE_METR.AsFloat);

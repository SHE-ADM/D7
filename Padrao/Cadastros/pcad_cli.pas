...
uses uPrincipal, bPrincipal,
  {$IFDEF DEF_COMPRAS} pcad_cli_edi {$ELSE} pcad_cli_edi {$ENDIF};
...
begin
  ...
  SIMPesquisa.Tag := 0 else
  begin
    oCTransact(TConsulta);
    oOTransact(TConsulta);
  end;

  if RECPrincipal.PSQTextoField = '_CDNF' then
  begin
    RECPrincipal.PSQTextoField := 'ID';
    RECPrincipal.PSQTexto      := bPesquisaNotasFiscais(RECPrincipal.PSQTexto);
  end;
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT CL.ID "Id",CLI_FANT "Fantasia",CLI_RAZA "Razao",');
    SQL.Add('       CLI_DCAD "DTCadastro",CLI_DFUN "DTFundacao",CLI_DALT "DTEdicao",CL.DTPV AS DTPEDIDO,');
    ...
    SQL.Add('       Vendedores.ID "IDVendedor",Vendedores.LOGIN "DEVendedor",Representantes.ID "IDRepresentante",Representantes.REP_FANT "DERepresentante",');
    SQL.Add('       CLI_DPAG "Prazo"     ,CL.IDCT "IDTransportadora",CLI_DTRA "DETransportadora",');
    ...
    SQL.Add('       IIF(CL.INFADCAD IS NULL,'''',TRIM(SUBSTRING(CL.INFADCAD FROM 1 FOR 812)) || ASCII_CHAR(13) || ASCII_CHAR(10)) ||');
    SQL.Add('       IIF(CL.INFADFIN IS NULL,'''',TRIM(SUBSTRING(CL.INFADFIN FROM 1 FOR 812)) || ASCII_CHAR(13) || ASCII_CHAR(10)) AS VARCHAR(1064))),'''') AS INFADCAD');

    SQL.Add('FROM      CAD_CLI     AS CL');
    SQL.Add('LEFT JOIN CAD_CLI_CRD AS FAVCredito     ON (CL.ID   = FAVCredito.IDCD AND FAVCredito.IDEP = ''' + RECParametros.EP_ID + ''')');
    SQL.Add('LEFT JOIN TAB_USER    AS Vendedores     ON (CL.IDCV = Vendedores.ID    )');
    SQL.Add('LEFT JOIN CAD_REP     AS Representantes ON (CL.IDCR = Representantes.ID)');

    SQL.Add('WHERE  CL.ID > 0');

    if Length(RECPrincipal.PSQTexto) >= 1 then
    SQL.Add('AND CL'+IFThen(RECPrincipal.PSQTextoField = 'ID','.ID','.CLI'+RECPrincipal.PSQTextoField)+' '+RECPrincipal.PSQWhere+' '''+Trim(RECPrincipal.PSQLike1+RECPrincipal.PSQTexto+RECPrincipal.PSQLike2)+'''');

    if not oEmpty(RECPrincipal.PSQData) then
    SQL.Add('AND CL.CLI'+RECPrincipal.PSQData+' BETWEEN '''+formatDateTime('mm/dd/yy',RECPrincipal.PSQDT1)+''' AND '''+formatDateTime('mm/dd/yy',RECPrincipal.PSQDT2)+'''');
...
begin
  ...
  begin
    ...
    SQL.Add('FROM      CAD_CLI     AS CL');
    SQL.Add('LEFT JOIN CAD_CLI_CRD AS FAVCredito     ON (CL.ID   = FAVCredito.IDCD AND FAVCredito.IDEP = :IDEmpresa)');
    SQL.Add('LEFT JOIN TAB_USER    AS Vendedores     ON (CL.IDCV = Vendedores.ID    )');
    SQL.Add('LEFT JOIN CAD_REP     AS Representantes ON (CL.IDCR = Representantes.ID)');

    if REC_SHE_DEF.IDPK = -3 then
       SQL.Add('WHERE '+IFThen(REC_SHE_DEF.FB_TB_PK = 'I','CL.ID = ','CL.CLI_FANT CONTAINING ')+''''+REC_SHE_DEF.ID+'''') else
    if AIDCliente > 0 then
       SQL.Add('WHERE CL.ID = ''' + IntToStr(AIDCliente) + '''') else
    begin
      SQL.Add('WHERE (CAST(CL.DTCA AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))) ');
      SQL.Add('OR     CAST(CL.DTPV AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))) ');
      SQL.Add('OR     CAST(CL.DTNF AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))))');

      if RECUsuarios.Grupo = 'VEN' then
         SQL.Add('AND CL.IDCV = ''' + RECUsuarios.Id + '''');
    end;

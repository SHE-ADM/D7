...
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT   PK.TITULO,PK.CDST,PK.DEST,PK.API_ST');
      ...
      if Eof then
      oException(EDCDNF,'Título(s) não Encontrdo(s) !');

      while not SQLConsulta.Eof do
      begin
        if (SQLConsulta.Current.ByName('CDST').AsInteger <> 116) and  { Pendente }
           (SQLConsulta.Current.ByName('CDST').AsInteger <> 127) then { Prorrogado }
        begin
          SBRodape.Panels[2].Text := 'Nossa Situação: ' + SQLConsulta.Current.ByName('DEST').AsString + '.' + #13 + #13 +
                                     'Somenete é permitido a emissão de títulos pendentes ou prorrogados.';
          Break;
        end;

        if (SQLConsulta.Current.ByName('API_ST').AsString <> EmptyStr) and            { Sem Emissão }
           (LeftStr(SQLConsulta.Current.ByName('API_ST').AsString,5) <> 'BAIXA') and  { Baixados }
           (LeftStr(SQLConsulta.Current.ByName('API_ST').AsString,5) <> 'DESCA') then { Descartados }
        begin
          SBRodape.Panels[2].Text := 'Situação Bancária: ' + SQLConsulta.Current.ByName('API_ST').AsString + '.' + #13 + #13 +
                                     'Somenete é permitido a emissão de títulos novos, descartados ou baixados departamento financeiro.';
          Break;
        end;
    ...
    oException(EDCDNF,'Falha ao tentar emitir título nº ' + SQLConsulta.Current.ByName('TITULO').AsString + ' !' + #13 +
                       SBRodape.Panels[2].Text + #13 + #13 +
                       'Favor entrar em contato com o departameno financeiro.');
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT   PK.TITULO,PK.API_ST,MAX(PK.DTVC) AS DTVC FROM ' + oREPZero('FIN_REC_BAN_BAI','_',RECParametros.EP_ID,3) + ' AS PK');
      SQL.Add('WHERE    PK.TITULO LIKE ''' + EDCDNF.Text + '%''');
      SQL.Add('AND     (PK.CDST = 116 OR PK.CDST = 127)');
      SQL.Add('GROUP BY PK.TITULO,PK.API_ST');
      ExecQuery;
    end;

    while not SQLConsulta.Eof do
    begin
      if   SQLConsulta.Current.ByName('API_ST').AsString <> 'BAIXADO' then
      with SQLFKConsulta do
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT PK.ID,PK.API_ST FROM FIN_REC_LOG_API AS PK');
        SQL.Add('WHERE  PK.TITULO = ''' + SQLConsulta.Current.ByName('TITULO').AsString + '''');
        SQL.Add('AND    PK.API_ST NOT LIKE ''BAIXADO%''');
        ExecQuery;
        ...
        begin
          ...
          while not Eof do
          begin
            if Current.Vars[1].AsString = 'DESCARTADO' then
            Break;
          ...
          if Current.Vars[1].AsString <> 'DESCARTADO' then
          oException(EDCDNF,'Boleto já emitido pelo banco !' + #13 +
                            'Título: ' + SQLConsulta.Current.ByName('TITULO').AsString + #13 + #13 +
      ...
      oException(EDCDNF,'Data de vencimento inválida !' + #13 +
  ...
  if i > 0 then
  if oYesNo(Application.Handle,'Parece que esse boleto já foi registrado .... ' + #13 +
                               'Tem certeza que deseja continuar ?') = mrNo then
  oException(EDCDNF,'Operação Cancelada !');
